...
<template>
  <div class="balancebox">
    ...
    <!-- 充值 -->
    <div class="editbox" v-if="topupFlag">
      <div class="batchtopupbox">
        <div class="stepone">
          <Form :model="formLeft" label-position="left" :label-width="70">
            <FormItem label="代理商">
              <Input v-model="formLeft.userName" disabled="disabled" style="width: 125px !important;"/>
            </FormItem>
            <FormItem label="当前余额">
              <Input v-model="formLeft.accountBalance" disabled="disabled" style="width: 125px !important;"/>
            </FormItem>
            <!-- 查询时无请选择套餐 -->
            <FormItem label="充值金额">
              <Input v-model="formLeft.topupMoney" placeholder="充值金额必须>=50" style="width: 125px !important;"/>
            </FormItem>
          </Form>
          <!-- 查询时无充值按钮-->
          <div class="btnbox">
            <Button type="primary" style="margin:0;">充值</Button>
            <Button @click="handleReset('formCustom')" style="margin-left: 8px">取消</Button>
          </div>
        </div>
      </div>
...
export default {
  data() {
    ...
    return {
      ...
      ruleCustom: {
        ...
        accountnumber: [{ validator: validateAge, trigger: "blur" }],
        phonenumber: [{ validator: validatePhonenumber, trigger: "blur" }],
        emails: [{ validator: validateemail, trigger: "blur" }]
      },
      loginAccount: "",
      userName: "",
      phoneNum: "",
  ...
  methods: {
    ...
    getapiList() {
      ...
      this.$fetch("/api/queryByType", params).then(function(data) {
        that.yidongpickers = data;
      });
    },
    //  套餐明细
    queryByList: params => {
      this.$fetch("/api/queryByType", params).then(data => {
    ...
    getList(params) {
      var that = this;
      this.$fetch("/user/queryByPage", params).then(function(data) {
